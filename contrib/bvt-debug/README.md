...
`bvt-debug` spawns a child process on a pseudo-terminal, pipes its
output through bloom-vt, and renders the resulting grid as plain text.
It is useful for:
...
- Validating that TUI applications render correctly through bloom-vt

The bloom-vt output callback is wired back to the PTY, so terminal
...
From a bloom-vt build tree:

```sh
./autogen.sh && ./configure && make
```

The `bvt-debug` binary is installed alongside the library.

## Usage

```
bvt-debug [options] [-- command args...]
```
...
```sh
bvt-debug -c 68 -r 20 -w 5 -- crush
```

**Send a prompt and capture raw output:**
```sh
bvt-debug -c 68 -r 20 -w 5 -i "hello world\n" -W 20 \
    -o /tmp/crush.raw -- crush
```

**Inspect just the status bar row with cell data:**
```sh
bvt-debug -c 68 -r 20 -w 5 -s 18 -d -q -- crush
```
...
gcc -o replay replay.c -lbloom-vt
...
4. Drains PTY output into `bvt_input_write()`
5. Wires `BvtCallbacks.output` back to the PTY so the child
   receives DA/DSR/kitty-keyboard responses
6. After the drain timeout, renders the bloom-vt grid as text
