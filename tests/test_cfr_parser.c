/* tests/test_cfr_parser.c — coffer parser smoke tests */
...
static void test_lf_cr(void)
{
    ...
    const CfrCell *c = cfr_get_cell(vt, 1, 0);
    ASSERT_EQ(c->cp, (uint32_t)'c');
    const CfrCell *d = cfr_get_cell(vt, 1, 1);
...
static void test_csi_cup(void)
{
    CfrTerm *vt = make_term(24, 80);
    feed(vt, "\x1b[5;10HX");
    const CfrCell *c = cfr_get_cell(vt, 4, 9);
    ASSERT_NOT_NULL(c);
...
static void test_csi_erase_line(void)
{
    ...
    const CfrCell *c = cfr_get_cell(vt, 0, 2);
    ASSERT_EQ(c->cp, 0x20u); /* erased — BCE blank (space) */
    const CfrCell *d = cfr_get_cell(vt, 0, 3);
...
static void test_osc8_uri_roundtrip(void)
{
    ...
    ASSERT_NEQ(c->hyperlink_id, 0);
    uint8_t uri[64];
    size_t n = cfr_cell_get_hyperlink(vt, c, uri, sizeof(uri));
...
static void test_osc8_id_cap(void)
{
    ...
    for (int i = 0; i < 256; ++i) {
        ...
        cfr_input_write(vt, (const uint8_t *)osc, (size_t)n);
    }
    ...
    const CfrCell *c = cfr_get_cell(vt, row, col);
    ASSERT_NOT_NULL(c);
    ASSERT_NEQ(c->hyperlink_id, 0);
    cfr_free(vt);
}
...
static void test_osc8_cjk_continuation(void)
{
    ...
    const CfrCell *lead = cfr_get_cell(vt, 0, 0);
...
static void test_osc8_scrollback_reintern(void)
{
    ...
    const CfrCell *l = NULL;
    for (int i = 0; i < sb; ++i) {
        const CfrCell *cell = cfr_get_scrollback_cell(vt, i, 0);
    ...
    uint8_t uri[64];
    size_t n = cfr_cell_get_hyperlink(vt, l, uri, sizeof(uri));
    ASSERT_EQ(n, (size_t)18);
    uri[n] = '\0';
    ASSERT_STR_EQ((const char *)uri, "http://example.com");
    cfr_free(vt);
...
static void test_csi_sgr_reset(void)
{
    ...
    ASSERT_NOT_NULL(cfr_get_cell(vt, 0, 0));
    ASSERT_NOT_NULL(cfr_get_cell(vt, 0, 1));
    cfr_free(vt);
}
...
static void test_style_intern_dedup(void)
{
    ...
    const CfrStyle *as = cfr_cell_style(vt, a);
    ASSERT_NOT_NULL(as);
    ASSERT_TRUE((as->attrs & CFR_ATTR_BOLD) != 0);
    cfr_free(vt);
}
...
static void test_grapheme_arena_dedup(void)
{
    ...
    feed(vt, "x");
    CfrPage *page = vt->grid;
    ASSERT_NOT_NULL(page);

    uint32_t flag[] = { 0x1F1E9, 0x1F1F0 }; /* DK flag */
    uint32_t id1 = cfr_grapheme_intern(vt, page, flag, 2);
    uint32_t id2 = cfr_grapheme_intern(vt, page, flag, 2);
    ASSERT_NEQ(id1, 0u);
    ASSERT_EQ(id1, id2);

    uint32_t fam[] = { 0x1F468, 0x200D, 0x1F469, 0x200D, 0x1F467, 0x200D, 0x1F466 };
    uint32_t id3 = cfr_grapheme_intern(vt, page, fam, 7);
    ASSERT_NEQ(id3, 0u);
    ASSERT_NEQ(id1, id3);

    uint32_t out[8] = { 0 };
    size_t n = cfr_grapheme_read(page, id3, out, 8);
    ASSERT_EQ(n, 7u);
...
static void test_width_cjk(void)
{
    ...
    const CfrCell *x = cfr_get_cell(vt, 0, 2);
...
static void test_width_emoji(void)
{
    ...
    feed(vt, "\xF0\x9F\x8E\x89"
             "y");
    const CfrCell *e = cfr_get_cell(vt, 0, 0);
    ASSERT_EQ(e->cp, 0x1F389u);
    ASSERT_EQ(e->width, 2);
    const CfrCell *y = cfr_get_cell(vt, 0, 2);
...
static void test_vs16_widens_ambiguous(void)
{
    ...
    feed(vt, "\xE2\x9A\xA0"
             "x");
    const CfrCell *plain = cfr_get_cell(vt, 0, 0);
    ...
    feed(vt, "\xE2\x9A\xA0\xEF\xB8\x8F"
             "x"); /* ⚠ + VS16 + x */
    const CfrCell *vs = cfr_get_cell(vt, 0, 0);
    ...
    uint32_t cps[8] = { 0 };
    size_t n = cfr_cell_get_grapheme(vt, vs, cps, 8);
    ASSERT_EQ(n, 2u);
...
static void test_combining_mark(void)
{
    ...
    feed(vt, "e\xCC\x81"
             "f");
    const CfrCell *eacute = cfr_get_cell(vt, 0, 0);
    ...
    const CfrCell *f = cfr_get_cell(vt, 0, 1);
    ASSERT_EQ(f->cp, (uint32_t)'f');

    uint32_t cps[4] = { 0 };
    size_t n = cfr_cell_get_grapheme(vt, eacute, cps, 4);
    ASSERT_EQ(n, 2u);
...
static void test_skin_tone(void)
{
    ...
    feed(vt, "\xF0\x9F\x91\x8B"
             "\xF0\x9F\x8F\xBD"
             "k");
    const CfrCell *wave = cfr_get_cell(vt, 0, 0);
    ASSERT_EQ(wave->cp, 0x1F44Bu);
    ASSERT_EQ(wave->width, 2);
    uint32_t cps[4] = { 0 };
    size_t n = cfr_cell_get_grapheme(vt, wave, cps, 4);
    ASSERT_EQ(n, 2u);
    ASSERT_EQ(cps[1], 0x1F3FDu);
    const CfrCell *k = cfr_get_cell(vt, 0, 2);
...
static void test_scrollback_push_read(void)
{
    ...
    const CfrCell *first = cfr_get_scrollback_cell(vt, 0, 0);
    ASSERT_NOT_NULL(first);
    ASSERT_EQ(first->cp, (uint32_t)'A');
    const CfrCell *fifth = cfr_get_scrollback_cell(vt, 0, 4);
    ASSERT_EQ(fifth->cp, (uint32_t)'A');

    /* Active grid row 0 is now BBBBB, row 1 is CCCCC, row 2 is DDDDD. */
    const CfrCell *row0 = cfr_get_cell(vt, 0, 0);
    const CfrCell *row1 = cfr_get_cell(vt, 1, 0);
    const CfrCell *row2 = cfr_get_cell(vt, 2, 0);
...
static void test_scrollback_grapheme_reintern(void)
{
    ...
    const CfrCell *fam = cfr_get_scrollback_cell(vt, 0, 0);
    ASSERT_NOT_NULL(fam);
    ASSERT_EQ(fam->cp, 0x1F468u);
    ASSERT_EQ(fam->width, 2);
    ...
    uint32_t cps[8] = { 0 };
    size_t n = cfr_cell_get_grapheme(vt, fam, cps, 8);
    ASSERT_EQ(n, 3u);
    ASSERT_EQ(cps[0], 0x1F468u);
    ASSERT_EQ(cps[1], 0x200Du);
...
static void test_send_key_arrow(void)
{
    CfrTerm *vt = make_term(24, 80);
    g_output_len = 0;
    cfr_send_key(vt, CFR_KEY_UP, CFR_MOD_NONE);
    ASSERT_STR_EQ(g_output_buf, "\x1b[A");
    g_output_len = 0;
    g_output_buf[0] = '\0';
    cfr_send_key(vt, CFR_KEY_LEFT, CFR_MOD_CTRL);
...
static void test_send_text_alt(void)
{
    CfrTerm *vt = make_term(24, 80);
    g_output_len = 0;
    cfr_send_text(vt, "x", 1, CFR_MOD_ALT);
...
static void test_altscreen_save_restore(void)
{
    ...
    const CfrCell *alt = cfr_get_cell(vt, 0, 0);
    ASSERT_EQ(alt->cp, 0u);
    ...
    const CfrCell *a = cfr_get_cell(vt, 0, 0);
    ASSERT_EQ(a->cp, (uint32_t)'A');
    const CfrCell *b = cfr_get_cell(vt, 1, 0);
    ASSERT_EQ(b->cp, (uint32_t)'B');
    cfr_free(vt);
}
...
static void test_altscreen_decsc_does_not_clobber_1049(void)
{
    ...
    CfrCursor mid = cfr_get_cursor(vt);
...
{
    CfrTerm *vt = make_term(4, 3);
    feed(vt, "AAA\r\nBBB\r\nCCC\r\nDDD");
    feed(vt, "\x1b[2;1H"); /* CUP row 2 */
    feed(vt, "\x1b[L");    /* IL 1 — insert blank line at row 2 */
    /* Result: AAA, _, BBB, CCC. (DDD shifted off bottom.) */
    ASSERT_EQ(cfr_get_cell(vt, 0, 0)->cp, (uint32_t)'A');
    ASSERT_EQ(cfr_get_cell(vt, 1, 0)->cp, 0x20u);
    ASSERT_EQ(cfr_get_cell(vt, 2, 0)->cp, (uint32_t)'B');
    ASSERT_EQ(cfr_get_cell(vt, 3, 0)->cp, (uint32_t)'C');
...
static void test_sgr_truecolor(void)
{
    ...
    const CfrCell *x = cfr_get_cell(vt, 0, 0);
    const CfrStyle *s = cfr_cell_style(vt, x);
    ASSERT_NOT_NULL(s);
...
static void test_sgr_truecolor_colon_form(void)
{
    ...
    CfrTerm *vt = make_term(24, 80);
    feed(vt, "\x1b[38:2:255:128:0mX");
    const CfrStyle *s = cfr_cell_style(vt, cfr_get_cell(vt, 0, 0));
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(s->fg_rgb, 0xFF8000u);
    ASSERT_TRUE((s->color_flags & CFR_COLOR_DEFAULT_FG) == 0);
    cfr_free(vt);
...
static void test_sgr_underline_color_itu(void)
{
    ...
    ASSERT_EQ(s->underline, CFR_UL_SINGLE);
    ASSERT_TRUE((s->color_flags & CFR_COLOR_DEFAULT_UL) == 0);
    /* No bg leak. */
    ASSERT_TRUE((s->color_flags & CFR_COLOR_DEFAULT_BG) != 0);
    cfr_free(vt);
}
...
static void test_sgr_underline_color_curly_orange(void)
{
    ...
    const CfrStyle *s = cfr_cell_style(vt, cfr_get_cell(vt, 0, 0));
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(s->underline, CFR_UL_CURLY);
...
static void test_sgr_underline_style_subparam_only(void)
{
    ...
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(s->underline, CFR_UL_CURLY);
    ASSERT_TRUE((s->attrs & CFR_ATTR_ITALIC) == 0);
    cfr_free(vt);

    CfrTerm *vt2 = make_term(24, 80);
    feed(vt2, "\x1b[4;3mI");
    const CfrStyle *s2 = cfr_cell_style(vt2, cfr_get_cell(vt2, 0, 0));
    ASSERT_NOT_NULL(s2);
    ASSERT_EQ(s2->underline, CFR_UL_SINGLE);
    ASSERT_TRUE((s2->attrs & CFR_ATTR_ITALIC) != 0);
    cfr_free(vt2);
}
...
static void test_reflow_overflow_to_scrollback(void)
{
    ...
    ASSERT_TRUE(cfr_get_scrollback_lines(vt) >= 1);
    cfr_free(vt);
}
...
static void test_reflow_preserves_styles(void)
{
     ...
     * B is default. */
    const CfrCell *a = cfr_get_cell(vt, 0, 0);
    const CfrCell *b = cfr_get_cell(vt, 0, 5);
    ASSERT_EQ(a->cp, (uint32_t)'A');
    ASSERT_EQ(b->cp, (uint32_t)'B');
    const CfrStyle *as = cfr_cell_style(vt, a);
...
{
    CfrTerm *vt = make_term(3, 10);
    feed(vt, "A\x1b[s\nB\x1b[uC");
    /* Row 0: A C ........ */
    ASSERT_EQ(cfr_get_cell(vt, 0, 0)->cp, (uint32_t)'A');
    ASSERT_EQ(cfr_get_cell(vt, 0, 1)->cp, (uint32_t)'C');
    /* Row 1: . B ........ */
    ASSERT_EQ(cfr_get_cell(vt, 1, 0)->cp, (uint32_t)0);
    ASSERT_EQ(cfr_get_cell(vt, 1, 1)->cp, (uint32_t)'B');
    CfrCursor c = cfr_get_cursor(vt);
    ASSERT_EQ(c.row, 0);
    ASSERT_EQ(c.col, 2);
    cfr_free(vt);
...
static void test_tbc_clear_all_then_ht(void)
{
    ...
    ASSERT_EQ(cfr_get_cell(vt, 0, 0)->cp, (uint32_t)'X');
    ASSERT_EQ(cfr_get_cell(vt, 0, 8)->cp, (uint32_t)'B');  /* untouched */
    ASSERT_EQ(cfr_get_cell(vt, 0, 16)->cp, (uint32_t)'C'); /* untouched */
    ASSERT_EQ(cfr_get_cell(vt, 0, 29)->cp, (uint32_t)'Y');
    cfr_free(vt);
}
...
static void test_tbc_clear_at_cursor(void)
{
    ...
    ASSERT_EQ(cfr_get_cell(vt, 0, 0)->cp, (uint32_t)'A');
    ASSERT_EQ(cfr_get_cell(vt, 0, 8)->cp, (uint32_t)0);
    ASSERT_EQ(cfr_get_cell(vt, 0, 16)->cp, (uint32_t)'B');
    cfr_free(vt);
}
...
static void test_dec_graphics_g0(void)
{
    ...
    ASSERT_EQ(cfr_get_cell(vt, 0, 0)->cp, 0x250Cu);
    ASSERT_EQ(cfr_get_cell(vt, 0, 1)->cp, 0x2500u);
    ASSERT_EQ(cfr_get_cell(vt, 0, 2)->cp, 0x2510u);
    cfr_free(vt);
}
static void test_dec_graphics_si_so(void)
{
    ...
    ASSERT_EQ(cfr_get_cell(vt, 0, 0)->cp, (uint32_t)'A');
    ASSERT_EQ(cfr_get_cell(vt, 0, 1)->cp, (uint32_t)'a');
    ASSERT_EQ(cfr_get_cell(vt, 0, 2)->cp, 0x2592u);
    ASSERT_EQ(cfr_get_cell(vt, 0, 3)->cp, 0x2502u);
...
static void test_wrap(void)
{
    ...
    const CfrCell *c5 = cfr_get_cell(vt, 1, 0);
 ...
 * Without DECAWM support, coffer always wrapped, causing the
...
static void test_decawm_off_no_wrap(void)
{
    ...
    ASSERT_EQ(cfr_get_cell(vt, 0, 4)->cp, (uint32_t)'X');
    ASSERT_EQ(cfr_get_cell(vt, 1, 0)->cp, 0u); /* row 1 untouched */
    cfr_free(vt);
}
...
static void test_decawm_off_wide_at_margin(void)
{
    ...
    ASSERT_EQ(cfr_get_cell(vt, 0, 3)->cp, 0x3042u);
...
static void test_irm_insert_char(void)
{
    ...
    ASSERT_EQ(cfr_get_cell(vt, 0, 0)->cp, (uint32_t)'A');
    ASSERT_EQ(cfr_get_cell(vt, 0, 2)->cp, (uint32_t)'C');
    ASSERT_EQ(cfr_get_cell(vt, 0, 3)->cp, (uint32_t)'X');
    ASSERT_EQ(cfr_get_cell(vt, 0, 4)->cp, (uint32_t)'D');
    ASSERT_EQ(cfr_get_cell(vt, 0, 5)->cp, (uint32_t)'E');
    cfr_free(vt);
}
...
static void test_irm_off_restores_replace(void)
{
    ...
    ASSERT_EQ(cfr_get_cell(vt, 0, 3)->cp, (uint32_t)'X');
    ASSERT_EQ(cfr_get_cell(vt, 0, 4)->cp, (uint32_t)'Y');
    ASSERT_EQ(cfr_get_cell(vt, 0, 5)->cp, (uint32_t)'E');
    cfr_free(vt);
}
...
static void test_irm_crush_pattern(void)
{
    ...
    ASSERT_EQ(cfr_get_cell(vt, 0, 2)->cp, (uint32_t)'m');
    ASSERT_EQ(cfr_get_cell(vt, 0, 3)->cp, (uint32_t)'s');
    ASSERT_EQ(cfr_get_cell(vt, 0, 4)->cp, (uint32_t)'m');
    ASSERT_EQ(cfr_get_cell(vt, 0, 7)->cp, (uint32_t)'d');
    cfr_free(vt);
}
...
static void test_irm_at_right_margin(void)
{
    ...
    ASSERT_EQ(cfr_get_cell(vt, 0, 3)->cp, (uint32_t)'C');
    cfr_free(vt);
}
...
int main(int argc, char *argv[])
{
    ...
    printf("Running coffer parser tests:\n");
